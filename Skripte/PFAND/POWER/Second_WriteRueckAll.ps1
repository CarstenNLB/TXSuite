...
$ZaehlS009   = 0
$ZaehlS004   = 0
# SPA 20200128 Deckungskennzeichen F bei VO nicht mehr schreiben, wieviele gibt es?
$ZaehlKzF009 = 0
$ZaehlKzF004 = 0
$KzFJaNein   = 0  # 0 = Nein, 1 = Ja
$CheckKzF    = 'F'
$PruefKzF    = ' '
...
Get-Content $DateiOut2 | ForEach-Object {
  ...
  $KzFJaNein    = 0
  $PruefKzF     = $Re2DeckVO
  if ($CheckKzF -ieq $PruefKzF) { #Ja ist ein F
   $KzFJaNein   = 1
  }
  # SPA 20200128 Deckungskennzeichen F bei VO nicht mehr schreiben
  if ($Re2LB -ieq $NLBVGL) { # nur NORDLB
      ...
      if ( ($Re2AK -ieq "99") -or 
        ...
        # SPA 20200128 - Fall Deckungskz bei F VO ider SI nicht moeglich, daher keine Betrachtung bei Ausplatzierungsmerkmal 99
        if ($Re2AK -ieq "99") { # die die raus sollen
         ...
         # SPA 20200128 - nicht mehr unbedingt, abhaengig vom Deckungskennzeichen 
         if ( $KzFJaNein -eq 0 ) { # Kein F, bedeutet Schreiben 
            if ( $ZaehlS009 -eq 0 ) { # nur vor der ersten Zeile den Header
                # Eine Datei sollte aber eine Kopfzeile besitzen ...
                echo $Header009
            } # nur vor der ersten Zeile den Header
         ...
         } else { # ein F, bedeutet Nichtschreiben
         	 $ZaehlKzF009++
         } # ein F, bedeutet Nichtschreiben
