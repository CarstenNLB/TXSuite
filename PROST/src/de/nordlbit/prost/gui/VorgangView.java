...
public class VorgangView extends javax.swing.JFrame 
{
    ...
    public VorgangView(String pvFilenameINI) 
    {
        ...
        try 
        {
            ...
            String lvMandant = new String();
            do
            {
              lvMandant = lvReader.getPropertyString("Mandanten", "Mandant" + lvMandantenNr, "Fehler");
              if (!lvMandant.equals("Fehler"))
              {
                System.out.println("Mandant: " + lvMandant);
            ...
            for (int lvLauf = 0; lvLauf < ivListeMandanten.size(); lvLauf++)
            {
                ...
                String lvID = lvReader.getPropertyString(lvMandant, "ID", "Fehler");
                if (lvID.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Keine ID in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("ID: " + lvID);

                String lvInstitutID = lvReader.getPropertyString(lvMandant, "InstitutID", "Fehler");
                if (lvInstitutID.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Keine InstitutID in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("InstitutID: " + lvInstitutID);
                lvNeuerMandant = new Mandant(lvID, lvMandant, lvInstitutID);
                lvListeNeuerMandanten.put(lvID, lvNeuerMandant);

                String lvUsername = lvReader.getPropertyString(lvMandant, "Username", "Fehler");
                if (lvUsername.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Kein Username in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("Username: " + lvUsername);
                lvDatenbankParameter.setUsername(lvUsername);       
                
                String lvPassword = lvReader.getPropertyString(lvMandant, "Password", "Fehler");
                if (lvPassword.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Kein Password in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("Password: " + lvPassword);
                lvDatenbankParameter.setPassword(StringCrypt.decodeROT13(lvPassword));                
                
                lvDatenbankname = lvReader.getPropertyString(lvMandant, "Datenbankname", "Fehler");
                if (lvDatenbankname.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Kein Datenbankname in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("Datenbank: " + lvDatenbankname);
                lvDatenbankParameter.setDatenbankname(lvDatenbankname);

                String lvIP =  lvReader.getPropertyString(lvMandant, "IP", "Fehler");
                if (lvIP.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Keine IP in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("IP: " + lvIP);
                lvDatenbankParameter.setIP(lvIP);
            
                String lvPort =  lvReader.getPropertyString(lvMandant, "Port", "Fehler");
                if (lvPort.equals("Fehler"))
                {
                    System.out.println("[" + lvMandant + "]Kein Port in der 'mandanten.ini'...");
                    System.exit(1);
                }
                System.out.println("Port: " + lvPort);
                lvDatenbankParameter.setPort(lvPort);
                
                DatenbankZugriff.getListeDatenbankParameter().put(lvMandant, lvDatenbankParameter);
            }
        }
        catch (Exception exp)
        {
        ...
        this.jLabelSystem.setText(lvDatenbankname);
