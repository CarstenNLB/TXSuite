...
public class RefiRegisterAZ6Passiv
{
  // Transaktionen
   ...
   * @param pvLogger
   */
  public RefiRegisterAZ6Passiv(SicherheitenDaten pvSicherheitenDaten, OutputDarlehenXML pvOutputDarlehenXML, Logger pvLogger)
  {
   ...
   */
  public void importDarlehen2Transaktion(Darlehen pvDarlehen, Vorlaufsatz pvVorlaufsatz, String pvKundennummer)
  {
    ...
    ivFinanzgeschaeft.setQuelle("AAZ6REFI");
    ...
    if (lvOkayDarlehen)
    {
      ...
      ivFinanzgeschaeftDaten.setAnwendungsnummer("30");
      // Kommentar
    ...
    if (lvOkayDarlehen)
    {
      ivKredkundeKonsorte.setKdnr(pvDarlehen.getCustomerID()); // CustomerID == Kundennummer im Fremdanteil
    ...
    if (lvOkayDarlehen)
    {
      ivKredkundeKreditnehmer.setKdnr(pvKundennummer); // Kundennummer der Nettozeile
    ...
    ivOutputDarlehenXML.printTransaktion(lvSicherheiten2RefiRegister.importSicherheitHypotheken(pvDarlehen.getKontonummer(), pvDarlehen.getKontonummer(), pvDarlehen.getCustomerID(), "1", new String(), "AAZ6REFI", pvVorlaufsatz.getInstitutsnummer(), null));//pvDarlehen.getKredittyp(), pvDarlehen.getBuergschaftProzent()));
