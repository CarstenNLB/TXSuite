...
public class TXSFinanzgeschaeftDaten implements TXSTransaktion
{
    ...
    // Hinzugefuegt 21.07.2020
    /**
     * Typ weitere Deckung - Wird fuer Termingelder benoetigt.
     */
    private String ivTypWeitereDeckung;
    ...
    public void initTXSFinanzgeschaeftDaten()
    {
        ...
        this.ivTypWeitereDeckung = new String();
    }
    ...
    /**
     * Liefert den Typ der weiteren Deckung
     */
    public String getTypWeitereDeckung()
    {
        return this.ivTypWeitereDeckung;
    }
    ...
    public StringBuffer printTXSTransaktionDaten()
    {
        ...
        if (this.ivKonto.length() > 0)
            lvHelpXML.append("konto=\"" + this.ivKonto + "\" ");
        ...
        if (this.ivTypWeitereDeckung.length() > 0)
        {
            lvHelpXML.append("typ_weitere_deckung=\"");
            lvHelpXML.append(this.ivTypWeitereDeckung);
            lvHelpXML.append("\" ");
        }
    ...
    public boolean importDarlehen(nlb.txs.schnittstelle.Darlehen.Daten.Extrakt.Darlehen pvDarlehen)
    {
        ...
        /* Nach Definition ohne Summe r�ckst�ndiger Leistung */
        ...
        /* �nderung vom 25.04.2005 Kontozustand > '8' statt > '4' */
        /* => �bernahme des Auszahlungstermines von == '4' auf > '3' */
        ...
        // Vor den Umsetzungen ... Pr�fungen, die zur Ablehnung f�hren
        //if (lvKTyp.isEmpty())
    ...
    public boolean importDarlehen(int pvModus, nlb.txs.schnittstelle.Darlehen.Daten.Extrakt.Darlehen pvDarlehen, Logger pvLogger)
    {
        ...
        // CT 25.02.2013
        // wenn (Schluessel Kompensation > 0 und < 16), dann "Konsortialkredit" = ja, sonst "Konsortialkredit" = nein
        // wenn (Schluessel Kompensation = 01/10/11), dann "Konsortialf�hrer" = NORD/LB, sonst
        // wenn (Schluessel Kompensation = 02/04/12/13/14/15), dann "Konsortialf�hrer" = Fremde Fuehrung, sonst "Konsortialfuehrer" bleibt leer
        this.ivKonskredit = "0"; // Standardmaessig erst einmal "0" setzen
        if (pvDarlehen.getInstitutsnummer().equals("009"))
        {
            if (StringKonverter.convertString2Int(pvDarlehen.getKompensationsschluessel()) > 0 &&
      ...
      if (lvKTyp.equals("undefiniert")) { // Kein Kredittyp moeglich !
        ...
        // Kennzeichen Rollover
        // Defaultm�ssig erst einmal auf "0" (false) setzen
        this.ivRoll = "0";
        // Bei Rollover muss das Kennzeichen gesetzt werden
        if (pvDarlehen.getKennzeichenRollover().equals("F") ||
            pvDarlehen.getKennzeichenRollover().equals("V"))
    ...
    public boolean importTermingeld(Termingeld pvTermingeld, Vorlaufsatz pvVorlaufsatz, Logger pvLogger)
    {
        ...
        this.ivBank = pvTermingeld.getGeschaeftspartnername();
        this.ivKonto = pvTermingeld.getKontonummer();
        this.ivText = pvTermingeld.getSchluesselHandeskat();
        ...
        this.ivRkapi = pvTermingeld.getSaldo();

        this.ivTyp = "62";

        this.ivAuspl = pvTermingeld.getAusplatzierungsmerkmal();
        this.ivAnwendungsnummer = "49";
        this.ivTypWeitereDeckung = "10";
